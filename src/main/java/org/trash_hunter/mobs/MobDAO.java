...
public class MobDAO extends DataAccessObject<MobDB> {
    ...
    private static final String INSERT = "INSERT INTO Mob (id,name,x,y) VALUES (?,?,?, ?)";
    private static final String GET_ONE = "SELECT name,x,y FROM Mob WHERE id = ?";
    private static final String UPDATE = "UPDATE Mob SET x=?,y=? WHERE id = ?";
    private static final String DELETE = "DELETE FROM Mob WHERE id = ?";
    private static final String GET_ALL = "SELECT id,name,x,y FROM Mob";
    ...
    public MobDB findById(long id) {
        ...
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(GET_ONE)) {
            ...
            if (resultSet.next()) {
                ...
                mobDB.setId(resultSet.getLong("id"));
    ...
    @Override
    public void update(MobDB mobDBUpdated, long id) {
    ...
    public void create(MobDB newmobDB) {
        try (PreparedStatement preparedStatement = this.connection.prepareStatement(INSERT, PreparedStatement.RETURN_GENERATED_KEYS)) {
            preparedStatement.setLong(1,newmobDB.getId());
